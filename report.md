...
*   **Prefect 3** - оркестратор, нужен в целом для запуска пайплайн и обработки ошибок.
...
1.  **Extract:** Скрипт выполняет HTTP GET-запрос к публичному API Open-Meteo. Для повышения надежности реализован механизм автоматических повторных попыток средствами Prefect, который перезапускает задачу при сетевых сбоях.
2.  **Load (Raw):** Полученные JSON-ответы сохраняются в неизменном виде в объектное хранилище **MinIO**. Файлы структурируются по датам и городам.
3.  **Transform:** Вложенные структуры JSON приводдятся к табличному виду с использованием pandas. Далее вычисляются суточные агрегаты (минимум, максимум, средняя температура и общее количество осадков)
4.  **Load (DWH):** Подготовленные данные загружаются в аналитическую СУБД **ClickHouse** в таблицы `weather_hourly` (детализация) и `weather_daily` (агрегаты).

После ETL происходит отправка сообщения в чат **Telegram**.

## Реализованные проверки
1.  **Валидация HTTP-ответов:** Во всех задачах, работающих с внешними API (Open-Meteo, Telegram), используется метод `response.raise_for_status()`. Это гарантирует, что пайплайн немедленно остановится, вместо того чтобы обрабатывать некорректные данные.
2.  **Идемпотентность загрузки:** В ClickHouse использован движок `ReplacingMergeTree` с ключами сортировки `(city, forecast_time)`. Дубликаты записей будут автоматически "схлопнуты" базой данных.
3.  **Сохранение "сырых" данных (Raw Layer):** Ответ API сохраняется в **MinIO** в неизменном виде. 
4.  **Автоматические повторные попытки:** Задачи Prefect настроены с параметром `retries=3`.

**Возможные точки сбоя:**
1.  **Изменение API:** Если Open-Meteo изменит структуру JSON-ответа.
2.  **API станет недоступным**
3.  **Диск переполнится**
...
![alt text](./image_report/image-1.png)

Результат выборки данных из таблицы **weather_daily**:

![alt text](./image_report/image-3.png)

###  Данные в MinIO

Демонстрация сохраненных JSON-файлов в бакете **weather-raw**:
![alt text](./image_report/image.png)

###  Уведомления в Telegram
Пример полученного прогноза:

![alt text](./image_report/image-2.png)

## Выводы

В ходе лабораторной работы был освоен полный цикл ETL-разработки. Для понимания работы докера и остальных иструментов использовались внешние источники (в частности использовалась AI модель gemini 3).

**Сложности и решения:**
1. Во время выполнения возникла проблема "Race Condition". Контейнер с Python-скриптом запускался быстрее сервера Prefect, что вызывало ошибку httpx.ConnectError. Решаем перезапуском (была идея сделать проверку перед запуском, но она не сработала).
...
**Что можно улучшить:**
1.  Добавить визуализацию и подключить с **ClickHouse**.
2.  Прикрутить строгую типизацию на этапе Extract.
4.  Добавить/сделать работающими Healthcheck-проверки для сервисов в Docker Compose, чтобы контейнер с пайплайном запускался строго после полной готовности базы данных.